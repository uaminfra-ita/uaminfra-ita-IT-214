...
function statusLabel(activity) {
  ...
  if (activity.type === 'break') return 'Calendário';
  return activity.checkpoint || activity.code;
}

export default function ActivityTimeline({ activities }) {
  const [filter, setFilter] = useState('all');
  const [today, setToday] = useState(null);
  const nextActivity = today ? nextScheduledActivity(activities, today) : null;
  const basePath = process.env.NEXT_PUBLIC_BASE_PATH || '';

  useEffect(() => setToday(courseDateKey()), []);
              ...
              <article className={`relative grid gap-5 pl-[72px] sm:grid-cols-[80px_1fr] sm:pl-0 ${isCancelled ? 'opacity-65' : ''}`} key={activity.code}>
                ...
                <div className="relative rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_18px_55px_-42px_rgba(7,20,38,.5)] sm:ml-8 sm:p-8">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <span className={`rounded-full px-3 py-1 text-[.65rem] font-black uppercase tracking-[.14em] ${isCancelled ? 'bg-red-100 text-red-800' : activity.type === 'break' ? 'bg-amber-100 text-amber-800' : activity.checkpoint ? 'bg-cyan-100 text-cyan-800' : 'bg-sky-100 text-sky-800'}`}>{statusLabel(activity)}</span>
