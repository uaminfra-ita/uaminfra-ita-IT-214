...
function StaffDashboard({ profile, membership, roster, onLogout }) {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <DashboardHeader name={profile.full_name} role={membership.role} onLogout={onLogout} />
      <div className="grid gap-4 sm:grid-cols-3"><div className="surface-card"><span className="text-4xl font-black text-cyan-700">{students.length}</span><p className="mt-2 text-sm font-bold text-slate-600">alunos no piloto</p></div><div className="surface-card"><span className="text-4xl font-black text-cyan-700">{staff.length}</span><p className="mt-2 text-sm font-bold text-slate-600">docentes no piloto</p></div><div className="surface-card"><span className="text-xl font-black text-ink">{nextActivity?.code || '—'}</span><p className="mt-2 text-sm font-bold text-slate-600">{nextActivity ? `${nextActivity.theme} · ${formatDate(nextActivity.date)}` : 'Sem encontros futuros'}</p></div></div>
      <section className="surface-card"><span className="eyebrow">Turma piloto</span><h3 className="mt-4 text-2xl font-black text-ink">Usuários cadastrados</h3><div className="mt-6 overflow-x-auto"><table className="w-full min-w-[560px] text-left text-sm"><thead><tr className="border-b border-slate-200 text-xs uppercase tracking-wider text-slate-400"><th className="pb-3">Nome</th><th className="pb-3">Papel</th><th className="pb-3">Estado</th></tr></thead><tbody>{roster.map((item) => <tr className="border-b border-slate-100 last:border-0" key={item.user_id}><td className="py-4 font-black text-ink">{item.full_name}</td><td className="py-4 text-slate-600">{item.role}</td><td className="py-4"><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-800">{item.status}</span></td></tr>)}</tbody></table></div></section>
