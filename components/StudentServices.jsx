...
function ArticleWorkspace({ latexUrl }) {
  ...
  const templates = resources.workspaceTemplates.map((template) => ({
    ...template,
    url: `${basePath}${template.assetPath}`,
  }));
  const templateLabels = {
    'it214-2026-latex-article-template': 'Projeto LaTeX IT-214',
    'elsevier-2024-elsarticle-template-bundle': 'Pacote Elsevier',
    'it212-2026-word-paper-template': 'Modelo Word',
  };
  ...
  return (
    <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-lift lg:col-span-2">
      <div className="grid lg:grid-cols-[.72fr_1.28fr]">
        ...
        <div className="p-7 sm:p-9"><span className="eyebrow">Seu workspace</span><h3 className="mt-4 text-xl font-black text-ink">Pasta “Projeto LaTeX”</h3><p className="mt-4 text-sm leading-6 text-slate-600">No projeto LaTeX IT-214, <code>main.tex</code> reúne os capítulos, a bibliografia fica em <code>referencias/</code> e as imagens em <code>figuras/</code>. O ZIP pode ser importado diretamente no Overleaf.</p><div className="mt-7 flex flex-wrap gap-3">{templates.map((template, index) => <a className={index === 0 ? 'button-dark' : 'button-primary'} href={template.url} download key={template.id}><Icon name="file" className="h-4 w-4" /> {templateLabels[template.id]}</a>)}<a className="button-primary" href={latexUrl} target="_blank" rel="noreferrer"><Icon name="external" className="h-4 w-4" /> Abrir projeto no Drive</a></div><p className="mt-5 text-xs leading-5 text-slate-500">O Drive armazena os arquivos, mas não compila LaTeX automaticamente. Envie também o PDF atualizado para facilitar a conferência.</p></div>
      </div>
    </section>
  );
}
